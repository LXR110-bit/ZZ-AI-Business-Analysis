...
> 飞书 base 是 source of truth，本地 `wiki_seed/*.json` 是镜像。两套脚本住 `scripts/` 下，分别是 push 和 pull。
...
> 场景：你在 git 上改了 JSON（修字段备注 / 加新字段），要推到飞书 base。

```bash
# 强烈建议先 --dry-run 看会发生什么
...
- 已知记录（`_record_id_map.json` 里有 record_id）→ `record-update`
- 新增记录（JSON 加了但 map 里没有）→ `record-create`，新 record_id 写回 map
- **永远不会删除飞书记录**（即使 JSON 里没了）。删请手动在飞书上做。

push 完后 `_record_id_map.json` 可能有更新（新建时），记得 `git add wiki_seed/_record_id_map.json && git commit`。

### 9.3 典型工作流

| 谁动了什么 | 跑哪个脚本 |
|---|---|
| 业务专家在飞书加新口径 | `pull` → 看 diff → commit |
| 你在 git 修字段备注 | `push --dry-run` → `push` |
| 仓库新搭、飞书 base 空 | `push`（一次性灌库） |
| 想撤销本地误改 | `git checkout wiki_seed/` 或重新 `pull` |

### 9.4 已知限制

- 默认走 `ssh zz-server`，因为 lark-cli 认证在那台机器上。本地直接跑需在本机装 lark-cli + 配认证
- 4 张表共约 129 条记录，全表 pull/push 约 1–2 分钟
- 03 表 `_所属字段` 是 text 不是 link，脚本不做翻译（直接当字符串）
- 04 表第一次 push 时 `_record_id_map.json` 没有 `table_04_definition_record_id_map` key，全走 create
- push 引用未知业务 ID（如 04 写了一个还没在 02 表里的 FLD999）→ RuntimeError 并提示先 pull
...
python3 scripts/tests/test_wiki_seed_pull.py      # 11 passed
python3 scripts/tests/test_wiki_seed_push.py      # 7 passed（含全表 dry-run smoke）
```
