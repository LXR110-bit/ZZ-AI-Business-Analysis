...
const BASE_SCRIPTS = ['category_daily_avg','category_summary','category_fulfill_daily_avg','category_fulfill_summary'];
const SCRIPTS = [...BASE_SCRIPTS,'model_daily_avg','model_summary'];
...
function resolveScriptScope(options={}){
 const sqlScope=String(options.sqlScope||'all').trim().toLowerCase();
 if(!['all','base'].includes(sqlScope))throw new Error(`sqlScope must be all or base, got ${options.sqlScope}`);
 const provided=Array.isArray(options.scripts)?options.scripts:(typeof options.scripts==='string'?options.scripts.split(','):[]);
 const scripts=(provided.length?provided:(sqlScope==='base'?BASE_SCRIPTS:SCRIPTS)).map(s=>String(s).trim()).filter(Boolean);
 if(new Set(scripts).size!==scripts.length)throw new Error(`scripts contains duplicates: ${scripts.join(',')}`);
 const expected=sqlScope==='base'?BASE_SCRIPTS:SCRIPTS;
 const unknown=scripts.filter(script=>!SCRIPTS.includes(script));
 const missing=expected.filter(script=>!scripts.includes(script));
 const extra=scripts.filter(script=>!expected.includes(script));
 if(unknown.length||missing.length||extra.length)throw new Error(`scripts do not match sqlScope=${sqlScope}; missing=${missing.join(',')||'<none>'}; extra=${extra.join(',')||'<none>'}; unknown=${unknown.join(',')||'<none>'}`);
...
function packageRawCache(options={}){
 ...
 const inputDir=path.resolve(options.inputDir||'.');const outDir=path.resolve(options.outDir||inputDir);const runId=options.runId||`fetch_${runDt}_${crypto.randomBytes(4).toString('hex')}`;const {sqlScope,scripts:activeScripts}=resolveScriptScope(options);ensureDir(outDir);
 ...
 try{ensureDir(path.join(tmp,'raw'));ensureDir(path.join(tmp,'sql'));const scripts={};const rawFiles=[];
  ...
  const sqlStatus={contract_version:CONTRACT_VERSION,stage:'fetch',run_id:runId,run_dt:runDt,sql_scope:sqlScope,active_scripts:activeScripts,status,scripts,generated_at:new Date().toISOString()};
  ...
  const rawManifest={contract_version:CONTRACT_VERSION,stage:'fetch',run_id:runId,run_dt:runDt,target_month:runDt.slice(0,7),sql_scope:sqlScope,scripts:activeScripts,raw_files:rawFiles,known_gaps:knownGaps,generated_at:new Date().toISOString()};
...
module.exports={packageRawCache,parseArgs,inspectCsv,sha256File,resolveScriptScope,BASE_SCRIPTS,SCRIPTS};
