...
def build_caches(imports_dir: Path, cache_dir: Path, run_dt: str, snapshot_dir: Path,
    ...
    board_csv = first_existing_file(snapshot_candidate_dirs(snapshot_dir), "board_metrics_feishu.csv")
    if board_csv:
        _, board_raw, _ = parse_csv_file(board_csv)
        board_rows = []
        for row in board_raw:
            ...
            if week:
                board_rows.append({"week": week, "startDate": start, "dau": _number_string(to_num(first(row, ("dau", "DAU", "APP日均DAU", "app_dau", "appDailyDau")))),
                                   "entryUv": _number_string(to_num(first(row, ("entryUv", "入口uv", "入口UV", "回收入口UV", "recycle_entry_uv"))))})
        board_cache = {"syncedAt": now_iso(), "version": "1.5.5-zloop", "source": {"prefixes": ["board_metrics", "board_metrics_feishu"], "file": str(board_csv.resolve())}, "weeks": sorted({row["week"] for row in board_rows}), "rows": board_rows}
    else:
        known_gaps.append("board_metrics_feishu.csv pending")
        board_cache = {"syncedAt": now_iso(), "version": "1.5.5-zloop", "source": {"prefixes": ["board_metrics", "board_metrics_feishu"], "targetWeeks": category_weeks}, "weeks": [], "rows": []}
    ...
    for category, rows in grouped.items():
        ...
        for metric in ("gmv", "dealCnt", "orderCnt", "evaUv"):
            ...
            if to_num(current.get("daysReceived")) >= to_num(previous.get("daysReceived")) and ratio < 0.5:
                ...
                if low_volume:
                    warnings.append(f"{message} (low_volume_baseline={_number_string(baseline)}, warn_only)")
                else:
...
def process_raw_cache(*, run_dt: str, input_dir: str | Path = ".", out_dir: str | Path | None = None, snapshot_dir: str | Path | None = None,
    ...
    try:
        ...
        quality_gate = "failed" if wtd["errors"] else ("warn" if warnings or known_gaps else "pass")
        quality_summary = {"quality_gates": quality_gate, "warnings": list(warnings), "known_gaps": list(known_gaps), "wtd_quality_errors": len(wtd["errors"])}
        ...
        quality_report = {"contract_version": "ai-wan-v1.5.5-quality", "run_id": process_id, "run_dt": run_dt, "generated_at": now_iso(), "quality_gates": quality_gate,
                          ...
                          "csv_repair": {key: value.get("csv_repair") for key, value in import_build["stats"].items()}, "wtd_quality": wtd,
                          ...
                          "category_mapping_manifest": caches["categoryMapping"], "tag_snapshot": tags["manifest"], "board_metrics": {"rows": len(caches["boardCache"].get("rows") or []), "gap": "board_metrics_feishu.csv pending" in known_gaps},
                          "warnings": warnings, "errors": wtd["errors"], "known_gaps": known_gaps}
