...
TERMINAL_FAILED = {"FAILED", "FAIL", "ERROR", "CANCELLED", "CANCELED", "TIMEOUT", "USER_CANCELED", "USER_CANCELLED"}
SQL_STATUS_ALIASES = {
    "CANCELLED": "CANCELED",
...
def normalize_sql_status(status: Any) -> str:
    """Return the canonical SQL execution status used by retry logic.

    Xinghe can return user-cancel variants such as ``USER_CANCELED`` while older
    gates only knew ``CANCELED``. Treating unknown terminal spellings as active
    causes Loop ticks to poll forever, so every checkpoint/poll path should use
    this helper before comparing against terminal sets.
    """
    canonical = str(status or "UNKNOWN").strip().upper()
    return SQL_STATUS_ALIASES.get(canonical, canonical)
...
def active_skill_dir() -> Path | None:
    ...
    # Runtime 未注入 ZLOOP_ACTIVE_SKILL_DIR 时，回退到脚本自身 __file__ 所在的技能根目录：
    # 这是确定性自定位（与 repo_root 一致），非搜索、不猜安装位置；仍禁止 find/locate。
    # env 有值时行为完全不变，仍会走 active_skill_dir_matches_script 校验捕获错误绑定。
    return repo_root()
...
def preflight() -> dict[str, Any]:
    ...
    required_files = [
        ...
        root / "scripts" / "package_raw_cache.py",
...
CSV_METRIC_ALIASES = {
    "jkuv": ("机况uv", "机况UV", "ji_kuang_uv", "jkuv", "jk_uv"),
    "evaUv": ("估价uv", "估价UV", "gu_jia_uv", "eva_uv", "evaUv"),
    "orderUv": ("下单uv", "下单UV", "xia_dan_uv", "order_uv", "orderUv"),
    "orderCnt": ("下单量", "xia_dan_cnt", "order_cnt", "orderCnt"),
    "shipCnt": ("发货量", "fa_huo_cnt", "ship_cnt", "shipCnt"),
    "signCnt": ("签收量", "qian_shou_cnt", "sign_cnt", "signCnt"),
    "qcCnt": ("质检量", "zhi_jian_cnt", "qc_cnt", "qcCnt"),
    "dealCnt": ("成交量", "cheng_jiao_cnt", "deal_cnt", "dealCnt"),
    "gmv": ("成交gmv", "成交GMV", "cheng_jiao_gmv", "deal_gmv", "gmv"),
}


def csv_metric_totals(csv_path: Path) -> dict[str, Any]:
    totals = {key: 0.0 for key in (*UV_METRICS, *ORDER_CHAIN_METRICS)}
    rows = 0
    if not csv_path.exists() or not csv_path.is_file():
        return {"row_count": 0, "totals": totals, "matched_columns": {}}
    with csv_path.open("r", encoding="utf-8-sig", errors="ignore", newline="") as f:
        reader = csv.DictReader(f)
        fieldnames = reader.fieldnames or []
        by_normalized = {header_key(name): name for name in fieldnames}
        matched: dict[str, str] = {}
        for metric, aliases in CSV_METRIC_ALIASES.items():
            for alias in aliases:
                hit = by_normalized.get(header_key(alias))
                if hit is not None:
                    matched[metric] = hit
                    break
        for row in reader:
            rows += 1
            for metric, column in matched.items():
                totals[metric] += numeric_value(row.get(column))
    return {"row_count": rows, "totals": totals, "matched_columns": matched}


def order_chain_integrity_from_totals(totals: dict[str, Any], row_count: int) -> dict[str, Any]:
    uv_total = sum(numeric_value(totals.get(key)) for key in UV_METRICS)
    order_chain_total = sum(numeric_value(totals.get(key)) for key in ORDER_CHAIN_METRICS)
    ok = not (row_count > 0 and uv_total > 0 and order_chain_total == 0)
    result = {"ok": ok, "row_count": row_count, "uv_total": uv_total, "order_chain_total": order_chain_total, "totals": totals}
    if not ok:
        result.update({
            "code": ORDER_CHAIN_EMPTY_CODE,
            "message": "UV is non-zero but order/deal/GMV metrics are all zero; data partition is likely not ready and must not be reused or published.",
        })
    return result


def csv_order_chain_integrity(csv_path: Path) -> dict[str, Any]:
    stats = csv_metric_totals(csv_path)
    result = order_chain_integrity_from_totals(stats["totals"], int(stats.get("row_count") or 0))
    result["csv"] = str(csv_path)
    result["matched_columns"] = stats.get("matched_columns", {})
    return result


class DataIntegrityRetryable(RuntimeError):
    def __init__(self, check: dict[str, Any]):
        self.check = check
        super().__init__(f"{check.get('code')}: {check.get('message')} totals={check.get('totals')}")
...
def validate_csv_for_reuse(csv_path: Path, allow_empty: bool) -> dict[str, Any] | None:
    ...
    if rows <= 0 and not allow_empty:
        return None
    integrity = csv_order_chain_integrity(csv_path)
...
def assert_csv_materialized_usable(csv_path: Path, allow_empty: bool) -> dict[str, Any]:
    reusable = validate_csv_for_reuse(csv_path, allow_empty)
    if reusable:
        return reusable
    integrity = csv_order_chain_integrity(csv_path)
    if not integrity.get("ok"):
        raise DataIntegrityRetryable(integrity)
    if not csv_path.exists() or not csv_path.is_file():
        raise RuntimeError(f"materialized CSV missing: {csv_path}")
    rows = csv_row_count(csv_path)
    if rows <= 0 and not allow_empty:
        raise RuntimeError(f"materialized CSV has no rows: {csv_path}")
    return {
        "row_count": rows,
        "csv": str(csv_path),
...
def render_sql(text: str, run_dt: str, data_end_date: str) -> str:
    rendered = text
    # ${outFileSuffix} 在模板里代表"数据日/昨日"（`set end_date = ${outFileSuffix}` 注释「代表昨日」），
    # 必须注入 data_end_date，而非 run_dt（今天）。否则周边界锚点(outFileSuffix)会比窗口末尾
    # ${hiveconf:end_date}(=data_end_date) 晚一天，与 $bash 订单分区日期脱钩。
    rendered = rendered.replace("${outFileSuffix}", data_end_date)
...
def get_status_for(resp: Any, execute_id: str) -> str:
    ...
    if not chosen:
        return "UNKNOWN"
    return normalize_sql_status(chosen.get("status") or chosen.get("state") or chosen.get("execute_status"))
...
def execute_read(args: argparse.Namespace, run_dir: Path) -> dict[str, Any]:
        ...
        if previous.get("status") == "SUCCESS" and previous.get("rendered_sql_sha256") == rendered_sha:
            ...
            if execute_id:
                try:
                    ...
                    reusable_after_materialize = assert_csv_materialized_usable(csv_path, name in FULFILL_OPTIONAL_EMPTY)
                    statuses[name].update(previous)
                    statuses[name].update({
                        "status": "SUCCESS",
                        "rendered_sql": str(rendered_path),
                        ...
                        "data_integrity": reusable_after_materialize.get("data_integrity"),
                    })
                    continue
                except Exception as exc:
    ...
    def complete_success(name: str) -> None:
        ...
        usable = assert_csv_materialized_usable(csv_path, allow_empty)
        ...
        if active:
            ...
            for name in list(active):
                ...
                if status in TERMINAL_SUCCESS:
                    ...
                    except DataIntegrityRetryable as exc:
                        ended = time.time()
                        statuses[name].update({
                            "status": "DATA_PENDING",
                            "error_code": ORDER_CHAIN_EMPTY_CODE,
                            "error_summary": str(exc),
                            "data_integrity": exc.check,
                            "ended_at": datetime.utcnow().isoformat(timespec="seconds") + "Z",
                            "duration_seconds": round(ended - float(statuses[name].get("started_ts", ended)), 3),
                        })
    ...
    if failed:
        if any(statuses.get(name, {}).get("error_code") == ORDER_CHAIN_EMPTY_CODE for name in failed):
            error = f"{ORDER_CHAIN_EMPTY_CODE}: order chain data is not ready for {failed}"
        else:
    ...
    pkg_cmd = [
        sys.executable,
        str(repo_root() / "scripts" / "package_raw_cache.py"),
        "--run-dt",
        args.run_dt,
        "--run-id",
        args.run_id,
...
def execute_process(args: argparse.Namespace, run_dir: Path, read_result: dict[str, Any]) -> dict[str, Any]:
    ...
    if proc.returncode != 0:
        ...
        process_error_code = "PROCESS_COMMAND_FAILED"
        try:
            parsed_failure = json.loads(proc.stdout)
            errors = ((parsed_failure.get("report") or {}).get("errors") or []) if isinstance(parsed_failure, dict) else []
            if errors and isinstance(errors[0], dict) and errors[0].get("code"):
                process_error_code = str(errors[0].get("code"))
        except Exception:
            pass
        write_json(process_dir / "process_failure_diagnostic.json", {
            "stage": "process",
        ...
        raise RuntimeError(f"{process_error_code}: process failed: returncode={proc.returncode}; stderr_tail={stderr_tail!r}; stdout_tail={stdout_tail!r}")
...
def _row_first(row: dict[str, Any], aliases: tuple[str, ...]) -> Any:
    by_key = {header_key(k): v for k, v in row.items()}
    for alias in aliases:
        hit = by_key.get(header_key(alias))
        if hit not in (None, ""):
            return hit
    return ""


def _model_csv_paths(run_dir: Path) -> list[Path]:
    roots = [run_dir / "read_exports", run_dir / "read_artifacts" / "raw", run_dir]
    for pattern in ("model_summary_*.csv", "model_summary.csv"):
        paths: list[Path] = []
        for root in roots:
            if root.exists():
        ...
        if paths:
            return paths
    paths = []
    for pattern in ("model_daily_avg_*.csv", "model_daily_avg.csv"):
        for root in roots:
            if root.exists():
                paths.extend(sorted(root.glob(pattern)))
    return paths
...
def _read_model_csv_rows(run_dir: Path, requested: set[str]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for path in _model_csv_paths(run_dir):
        with path.open("r", encoding="utf-8-sig", errors="ignore", newline="") as f:
            reader = csv.DictReader(f)
            for raw in reader:
                if not isinstance(raw, dict):
                    continue
                week_start = str(_row_first(raw, ("week_start_date", "周开始", "开始日期", "统计日期", "日期", "startDate"))).strip()
                category = str(_row_first(raw, ("品类名称", "品类", "三级品类", "cate_name", "cate_name_label", "category", "category_name"))).strip()
                model_id = re.sub(r"^(\d+)\.0+$", r"\1", str(_row_first(raw, ("机型id", "机型ID", "型号ID", "model_id", "model_id_col", "modelId"))).strip())
                model_name = str(_row_first(raw, ("机型名称", "型号名称", "型号", "model_name", "model_name_label", "modelName"))).strip()
                if requested and category not in requested:
                    continue
                if not week_start or not category or not model_id:
                    continue
                item = {"week_start_date": week_start, "category": category, "model_id": model_id, "model_name": model_name}
                item["daysReceived"] = numeric_value(_row_first(raw, ("day_cnt", "daysReceived", "已收到天数")))
                for metric, aliases in CSV_METRIC_ALIASES.items():
                    item[metric] = numeric_value(_row_first(raw, aliases))
                rows.append(item)
    return rows


def _aggregate_model_rows(rows: list[dict[str, Any]]) -> dict[tuple[str, str, str], dict[str, Any]]:
    grouped: dict[tuple[str, str, str], dict[str, Any]] = {}
    for row in rows:
        key = (str(row["week_start_date"]), str(row["category"]), str(row["model_id"]))
        ...
        if row.get("model_name") and not out.get("model_name"):
            out["model_name"] = row["model_name"]
        out["daysReceived"] = max(numeric_value(out.get("daysReceived")), numeric_value(row.get("daysReceived")))
        for metric in ("jkuv", "evaUv", "orderUv", "orderCnt", "shipCnt", "signCnt", "qcCnt", "dealCnt", "gmv"):
            out[metric] = numeric_value(out.get(metric)) + numeric_value(row.get(metric))
    return grouped


def load_model_rows_for_categories(run_dir: str | Path, categories: list[Any]) -> list[dict[str, Any]]:
    """Parse Loop2 materialized model CSVs into deterministic candidate rows.

    Returns one row per requested category/model for the latest materialized week,
    with current metrics plus previous-week deltas. Missing previous rows yield
    zero baselines rather than fabricated pct movement.
    """
    requested = {
        str(c.get("category") if isinstance(c, dict) else c).strip()
        for c in (categories or [])
        if str(c.get("category") if isinstance(c, dict) else c).strip()
    }
    raw_rows = _read_model_csv_rows(Path(run_dir), requested)
    if not raw_rows:
        return []
    grouped = _aggregate_model_rows(raw_rows)
    weeks = sorted({key[0] for key in grouped})
    cur_week = weeks[-1]
    prev_week = weeks[-2] if len(weeks) >= 2 else None
    out: list[dict[str, Any]] = []
    for (week_start, category, model_id), cur in sorted(grouped.items()):
        if week_start != cur_week:
            continue
        prev = grouped.get((prev_week, category, model_id), {}) if prev_week else {}
        cur_gmv = numeric_value(cur.get("gmv"))
        prev_gmv = numeric_value(prev.get("gmv"))
        cur_deal = numeric_value(cur.get("dealCnt"))
        prev_deal = numeric_value(prev.get("dealCnt"))
        cur_rates = rates(cur)
        prev_rates = rates(prev)
        out.append({
            "category": category,
            "model_id": model_id,
            "model_name": cur.get("model_name") or prev.get("model_name") or model_id,
            "week_start_date": cur_week,
            "prev_week_start_date": prev_week,
            "gmv": cur_gmv,
            "dealCnt": cur_deal,
            "orderUv": numeric_value(cur.get("orderUv")),
            "shipCnt": numeric_value(cur.get("shipCnt")),
            "daysReceived": numeric_value(cur.get("daysReceived")),
            "gmv_delta": cur_gmv - prev_gmv,
            "gmv_delta_pct": ((cur_gmv - prev_gmv) / prev_gmv) if prev_gmv > 0 else None,
            "deal_delta": cur_deal - prev_deal,
...
def read_published_display(args: argparse.Namespace) -> dict[str, Any]:
    """Read the currently published Loop1 dashboard display for Loop2 merge."""
    response = hub_post(READ_PATH, {
        "run_id": getattr(args, "run_id", None) or f"loop2-{args.week}-{args.data_end_date}-r{getattr(args, 'base_revision', 1)}",
        "week": args.week,
        "stage": "validate",
        "include": ["metric_snapshot"],
    }, timeout=90.0)
    snapshot = ((response.get("context") or {}).get("metric_snapshot") or {}) if isinstance(response, dict) else {}
    status = snapshot.get("analysisStatus") if isinstance(snapshot, dict) else {}
    if isinstance(status, dict):
        expected_revision = getattr(args, "base_revision", None)
        actual_revision = status.get("base_revision")
        if expected_revision is not None and actual_revision is not None and int(actual_revision) != int(expected_revision):
            raise RuntimeError(f"BASE_DISPLAY_REVISION_MISMATCH: expected {expected_revision}, got {actual_revision}")
    insights = snapshot.get("insights") if isinstance(snapshot, dict) else None
    return insights if isinstance(insights, dict) else {}


def execute_model_validate(args: argparse.Namespace, run_dir: Path, merged_display: dict[str, Any]) -> dict[str, Any]:
    """Atomically publish Loop2 modelDrilldowns via the AIWAN validate bridge."""
    processed_path = run_dir / "processed_models.json"
    processed = read_json(processed_path) if processed_path.exists() else {"status": "success", "week": args.week}
    validation_result = {
        "stage": "validate",
        "status": "success",
    ...
    analysis_result = {
        ...
        "display_insights": merged_display,
        "modelDrilldowns": merged_display.get("modelDrilldowns") if isinstance(merged_display, dict) else {},
        "warnings": merged_display.get("warnings", []) if isinstance(merged_display, dict) else [],
    }
    envelope = {
        "run_id": args.run_id,
    ...
    reread = hub_post(READ_PATH, {
        "run_id": args.run_id,
        "stage": "validate",
        "week": args.week,
        "include": ["run_meta", "current_stage_output", "metric_snapshot"],
    }, timeout=90.0)
    ...
    insights = metric_snapshot.get("insights") if isinstance(metric_snapshot, dict) else None
    if (
        ...
        or analysis_status.get("analysis_key") != args.analysis_key
        or analysis_status.get("data_end_date") != args.data_end_date
        or int(analysis_status.get("base_revision") or 0) != int(args.base_revision)
        or analysis_status.get("model_enrichment_mode") != "enabled"
        or not isinstance(insights, dict)
        or insights.get("modelDrilldowns") != merged_display.get("modelDrilldowns")
    ):
        raise RuntimeError("MODEL_VALIDATE_REREAD_MISMATCH: server write/readback did not match Loop2 model drilldown validate payload")
    ...
    write_json(run_dir / "model_validation_result.json", validation_result)
...
def execute_validate(args: argparse.Namespace, run_dir: Path, processed: dict[str, Any], analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    dq = processed.get("data_quality_report") if isinstance(processed, dict) else {}
    dq_errors = dq.get("errors") if isinstance(dq, dict) else []
    manifest = processed.get("active_process_manifest") if isinstance(processed, dict) else {}
    process_failed = isinstance(manifest, dict) and manifest.get("quality_gates") == "failed"
    data_integrity_failed = any(isinstance(item, dict) and item.get("code") == ORDER_CHAIN_EMPTY_CODE for item in (dq_errors or []))
    check("process_data_integrity_no_order_chain_empty", not (process_failed or data_integrity_failed))
    ...
    envelope = {
        ...
        "payload": {
            ...
            "validation_result": validation_result,
            "publication_bundle": {
                "category_cache": read_cache_json(processed, "category-cache.json"),
                "category_taxonomy": read_cache_json(processed, "category-taxonomy.json"),
                "board_metrics": read_cache_json(processed, "board-metrics.json"),
                "source_manifest": read_cache_json(processed, "dashboard-source-manifest.json"),
            },
        },
    }
    if publish_allowed:
