...
def build_snapshot(records: list[dict], taxonomy: set[str], *, version: str, column_map: dict | None = None) -> dict:
    ...
    if report["conflicts"]:
        status = "conflict"   # 跨品类 model_id 冲突，拒绝升级快照（设计 §6.3）
    elif normalized:
        status = "ok"
    else:
        status = "pending_business_sheet"
    return {
...
def main() -> int:
    ...
    parser.add_argument("--force", action="store_true", help="即使存在跨品类冲突也强制落盘（默认拒绝，设计 §6.3）")
    ...
    if report["conflicts"] and not args.force:
        print("⚠️ 存在跨品类 model_id 冲突，拒绝升级快照（未落盘）。修正 sheet 后重跑，或 --force 强制落盘。", file=sys.stderr)
        return 2
    if args.dry_run:
        return 0
    if report["conflicts"]:
        print("⚠️ --force：带跨品类冲突强制落盘，status=conflict。", file=sys.stderr)
