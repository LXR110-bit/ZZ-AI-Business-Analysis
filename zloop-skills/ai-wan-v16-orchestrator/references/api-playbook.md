...
调用详情见 `references/loop1-control-plane-contract.md`。Loop1 只通过这两个 capability 做跨 tick 恢复、CAS 租约、SQL checkpoint 和发布复读；handoff / Loop2 仅作为预留能力，不纳入当前上线闭环。

### full7 兼容入口

`aiwan_inline_state_machine.py` 保留 full7 read/write 代码路径用于兼容回归，但它不是阶段 A Loop1 控制面。正式阶段 A 调度不得从旧 `/api/aiwan/read|write` 推断 checkpoint 状态。
...
```text
read 阶段：禁止 AIWAN read/write；委托 xinghe-data-explore 执行 7 份 SQL，生成 raw_cache/read_result。
process 阶段：禁止 AIWAN read/write；消费 raw_cache；`sqldau` 生成 board metrics，品类映射仍读取飞书/快照；生成 processed_data/category_mapping_manifest。
