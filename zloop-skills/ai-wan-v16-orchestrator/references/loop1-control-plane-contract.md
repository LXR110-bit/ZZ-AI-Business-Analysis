...
- `action=create`：创建或幂等复用 base/handoff job；重复 revision 的完整 payload 必须一致，否则按业务冲突失败。
...
- 新 base revision 或同周更新数据日到达时，旧活跃 base/handoff 立即 `superseded`。
...
Loop1 只允许：`category_daily_avg`、`category_summary`、`category_fulfill_daily_avg`、`category_fulfill_summary`。

## 发布与交接

1. validate 写入基础结果。
2. 复读 stage revision 及 dashboard `analysisStatus.analysis_key/data_end_date/base_revision`。
3. `model_enrichment_mode=disabled` 时 dashboard 必须为 `base_published`，不设 `model_sla_deadline`。
4. base job 通过 `jobs/write action=state` 标记 `published`，再用 `jobs/read` 复读。
5. 幂等创建 `kind=drilldown, handoff_revision=1, status=ready, model_enrichment_mode=disabled, sla_deadline=null`。
