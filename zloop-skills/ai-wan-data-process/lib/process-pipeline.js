...
function buildCaches(importsDir, cacheDir, runDt, snapshotDir, previousCacheDir, warnings, knownGaps) {
  ...
  const fulfillRows = mergeRowsByKey(fulfillRowsRaw.map((r) => ({ category: r['品类名称'], fulfillmentMethod: r['履约方式（只取线上流程）'], ...normalizeMetricRow(r, headersFor('category_fulfill_daily_avg'), runDt) })).filter((r) => r.week && r.category && !selfCats.has(r.category)), (r) => [r.week, r.category, r.fulfillmentMethod].join('\u001f'));
  ...
  return { taxonomy, categoryCache, fulfillCache, modelCache, boardCache };
}
...
async function processRawCache(options = {}) {
  ...
  try {
    ...
    const caches = buildCaches(processedImports, cacheDir, runDt, snapshotDir, previousCacheDir, warnings, knownGaps);
