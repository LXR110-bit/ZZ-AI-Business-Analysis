...
function packageRawCache(options={}){
 ...
 try{ensureDir(path.join(tmp,'raw'));ensureDir(path.join(tmp,'sql'));const scripts={};const rawFiles=[];
  for(const script of SCRIPTS){const csv=findInputFile(inputDir,script,'csv');if(!csv)throw new Error(`missing raw csv for ${script} in ${inputDir}`);const sql=findInputFile(inputDir,script,'sql');const rawRel=`raw/${script}_${runDt}.csv`;const sqlRel=`sql/${script}_${runDt}.sql`;fs.copyFileSync(csv,path.join(tmp,rawRel));let sqlText=sql?fs.readFileSync(sql,'utf8'):`-- SQL text unavailable for ${script}; packaged by package-raw-cache.js\n`;fs.writeFileSync(path.join(tmp,sqlRel),sqlText,'utf8');const csvInfo=inspectCsv(path.join(tmp,rawRel));scripts[script]={execute_id:options.executeId||'',status:csvInfo.row_count>0?'SUCCESS':'FAILED',row_count:csvInfo.row_count,column_count:csvInfo.column_count,bytes:csvInfo.bytes,sha256:csvInfo.sha256,raw_csv:rawRel,rendered_sql:sqlRel,rendered_sql_sha256:sha256Text(sqlText),started_at:options.startedAt||'',finished_at:options.finishedAt||new Date().toISOString(),error_summary:csvInfo.row_count>0?'':'empty csv'};rawFiles.push({script,path:rawRel,...csvInfo});}
  const status=Object.values(scripts).every(s=>s.status==='SUCCESS')?'success':'failed';
  ...
  const rawManifest={contract_version:CONTRACT_VERSION,stage:'fetch',run_id:runId,run_dt:runDt,target_month:runDt.slice(0,7),raw_files:rawFiles,known_gaps:options.knownGaps?String(options.knownGaps).split(',').filter(Boolean):[],generated_at:new Date().toISOString()};
  ...
  return {ok:status==='success',active_manifest:active,raw_cache:rawCache,sql_status:sqlStatus,raw_manifest:rawManifest};
