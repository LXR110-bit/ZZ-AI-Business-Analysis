...
## analysis_history 字段口径（防误判）

Process v1.5.5 输出的 `analysis_history_<run_dt>.json` 使用规范化 camelCase 指标字段，不使用中文指标名。Analyze 生成 evidence 前必须按以下字段读取，不得因为找不到中文字段而判定全零：

| 业务指标 | analysis_history 字段 |
|---|---|
| 机况 UV | `jkuv` |
| 估价 UV | `evaUv` |
| 下单 UV | `orderUv` |
...
判定 `all_data_zero_in_analysis_history` 前，必须至少汇总以下数组的 `gmv/dealCnt/orderCnt/evaUv/jkuv/orderUv`：
...
只有这些数组均为空，或上述字段汇总后核心指标均为 0，才允许登记 `all_data_zero_in_analysis_history`。如果字段存在且任一核心指标非零，禁止写 all-zero gap；应按实际非零数据生成 evidence。

`model_topn_history` 可能只包含 `gmv/dealCnt/orderCnt`，没有 UV 字段；不得因此判定 model 维度全零。
