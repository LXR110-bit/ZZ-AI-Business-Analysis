...
**日期**：2026-07-06
**基底分支**：`feature/model-tag-monitor-v2`（从 main 切出，PR #12 已 squash merge 进 main）
**三个并行子分支**：`feature/v2-backend`（backend-agent）/ `feature/v2-analysis`（analysis-agent）/ `feature/v2-frontend`（frontend-agent），均以 `feature/model-tag-monitor-v2` 为基底，完成后各自开 PR 合并回该基底分支
...
## 【2026-07-06 补充确认】品类/大盘维度漏斗数据来源（月度分表架构）

与"品类分层映射"（下一节，来自品类映射表.xlsx → 飞书 Base 单表）是**两套不同的数据**，不要混淆：
- 本节说的是**品类/大盘的漏斗指标**（GMV、机况UV、估价UV……），用于生成 `category-cache.json` 和大盘层数据
- 下一节"品类分层映射"是**品类归属哪个阶段**（发展/孵化/种子），用于生成 `category-taxonomy.json`

品类/大盘维度漏斗数据是 PR #39（机型周数据从飞书 Sheets 迁移到多维表格 Base）产物的延伸：飞书侧按**月度分表**，机型/品类/大盘三个维度各自都有独立的"汇总表"（周累计）和"日均表"（周日均）。**每个月都会新建一张表**，backend-agent 的同步逻辑必须设计成可扩展的"月份 → wiki node token"映射，不能像老 `src/sync.js` 那样硬编码单一 token。

**取用口径确认**：品类层和大盘层的漏斗数据都同步自**日均表**（与机型层 `src/sync.js` 现有的"周日均"口径一致，保证四层转化率公式口径统一）。

已知 wiki node token（下方按维度/月份列出，backend-agent 接入时以此为准，7月及以后新增月份需要在代码里补充映射，不确定的 token 要在真实同步前用 wiki `get_node` 接口核验 `obj_type`/文档标题，参照 `src/sync.js` 里 `EXPECTED_DOC_TITLE` 的校验模式）：

| 维度 | 粒度 | 月份 | wiki node token | table id |
|---|---|---|---|---|
| 品类 | 日均 | 4月 | `WapgwtEW2iRGGPkcItyclRRMnCc` | `tblaWu89uXoXuNXQ` |
| 品类 | 日均 | 5月 | `Vxluw1KQuilCfxkNEn4cPmVHnyh` | `tbl7MdKlLalygD4o` |
| 品类 | 日均 | 6月 | `OR4xw9xnwiZiXkke49BcJdydnLe` | `tblp4djUOMmE9gvS` |
| 品类 | 日均 | 7月 | `DAcFwVw8ViG3PHkqUOUcbmYGnDc` | `tbl5EZ8oGsVE8joQ` |
| 大盘 | 日均 | 5月 | `N8Ijw3rY7iIXhhkVRT6cZ9xTnPe` | `tblaByOmnpGBUVPo` |
| 大盘 | 日均 | 6月 | `N4bIw142pimzZDkCoPLc2Gt8nNc` | `tblM3nCspImZxNcP` |
| 大盘 | 日均 | 7月 | `OdhJwIkyvi55wVkPVxocZ7RFnIQ` | `tblj4hmOhxDt9bk4` |

（机型维度按月分表也存在，但**机型层同步逻辑保持不变**——`src/sync.js` 继续读老的单一连续 wiki 文档 `UzEZwrOTVimV0RkjOaBcT4EWnGf`，不切换到月度分表架构，避免无谓改动已稳定运行的机型层管线。）

**大盘层口径变更（与本文档下方"四层聚合逻辑"一节原定口径不同，以本节为准）**：大盘层原计划是"发展+孵化+种子三层求和"推导，现在飞书侧已有权威的大盘日均表，改为**直接同步大盘表作为大盘层数据**。原来的三层求和逻辑**保留**，改为**对账校验**用途：analysis-agent 计算三层求和结果与同步来的大盘表数据做比对，差异超过阈值时输出告警（不阻塞展示，只用于发现数据口径分歧）。
...
映射表同步链路：用户在本机维护 Excel → 上传到飞书 Base（多维表格）→ backend-agent 新增定时同步任务（参考现有 `src/sync.js` 模式）定期拉取落地为 `data/category-taxonomy.json`。

飞书 Base 地址：`https://zhuanspirit.feishu.cn/wiki/L7LowLNAbif0fgkzxIJcHCZynnb?table=tblXJ78kOrgKgZlc&view=vew54vaU0N`
- Wiki node：`L7LowLNAbif0fgkzxIJcHCZynnb`
- Table ID：`tblXJ78kOrgKgZlc`
- View ID：`vew54vaU0N`

backend-agent 需要用这个 wiki node 换取 obj token（多维表格用 Bitable records API，不是 Sheets API，接口形态与现有 `src/feishu.js` 的 Sheets 读取不同，需要新增 Bitable 专用的读取方法）。
...
### `data/board-cache.json`（backend-agent 产出，大盘层权威数据，日均口径）

```json
{
  "syncedAt": "ISO时间",
  "source": { "wikiNodesByMonth": { "2026-05": "N8Ijw3rY7iIXhhkVRT6cZ9xTnPe", "2026-06": "N4bIw142pimzZDkCoPLc2Gt8nNc", "2026-07": "OdhJwIkyvi55wVkPVxocZ7RFnIQ" } },
  "rows": [
    {
      "week": "2026-W27",
      "jkuv": 0, "evaUv": 0, "evaCnt": 0, "orderUv": 0, "orderCnt": 0,
...
无 `category` 字段（大盘层本身就是最高汇总层级），字段命名和派生比率公式与 `category-cache.json` 一致。
...
- **大盘层**：直接使用 backend-agent 同步的 `data/board-cache.json`（飞书大盘日均表，权威数据）。发展+孵化+种子三层求和结果**保留用于对账校验**：与大盘表数据比对，GMV 差异超过 5% 时输出告警字段（不阻塞展示，供人工排查数据口径分歧）
