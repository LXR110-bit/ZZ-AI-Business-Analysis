...
test('board.cur 只含 gmv + 4 rates，无计数字段', () => {
  const { board } = composeDashboard(baseOpts);
  const keys = Object.keys(board.cur).sort();
  assert.deepEqual(keys, ['dealRate', 'evaRate', 'gmv', 'orderRate', 'shipRate']);
});
...
test('tiers[].cur 只含精简字段 + categoryCount', () => {
  const { tiers } = composeDashboard(baseOpts);
  const expectedKeys = ['categoryCount', 'dealRate', 'evaRate', 'gmv', 'orderRate', 'shipRate'];
  for (const t of tiers) {
...
test('categories[].cur 只含精简字段', () => {
  const { categories } = composeDashboard(baseOpts);
  const expectedKeys = ['dealRate', 'evaRate', 'gmv', 'orderRate', 'shipRate'];
