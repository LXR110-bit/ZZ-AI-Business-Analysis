...
const { COUNT_KEYS, calcRates, calcDelta } = require('./funnel');
...
// 非加总字段：只在品类行原样透传，tier/board 不做 sum。
const PASSTHROUGH_KEYS = ['recycleEntranceUv'];

function buildCur(row) {
  if (!row) {
    const out = {};
    for (const k of COUNT_KEYS) out[k] = null;
    for (const k of PASSTHROUGH_KEYS) out[k] = null;
  ...
  for (const k of PASSTHROUGH_KEYS) counts[k] = row[k] === undefined ? null : row[k];
