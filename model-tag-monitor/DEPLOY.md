...
如果目标主机只能经公司堡垒机访问，且不能直接从本机 `rsync` 到目标机，用办公电脑临时 HTTP 服务让目标机反向下载文件：

```bash
# 1. 在待上传文件所在目录启动本机临时服务，端口必须是 23456
python3 -m http.server 23456

# 2. 获取办公网 IP。目标侧安全组约定放通 10.238.0.0/16 或 10.240.0.0/16 的 TCP 23456
ifconfig | awk '/inet / {print $2}' | grep -E '^(10\.238\.|10\.240\.)'

# 3. 经堡垒机登录目标主机后，在目标主机下载
wget http://<office_ip>:23456/<filename>
```

注意：2026-07-16 本机实测办公网来源 IP 为 `10.242.26.7`，但当前安全组说明只写了 `10.238.*`/`10.240.*`。如果目标机 `wget` 拉不通，先切换到放通网段的办公网/VPN，或请运维确认是否也放通 `10.242.0.0/16`。
