#!/usr/bin/env bash
# model-tag-monitor v1.3.0 daily refresh flow
...
# ecosystem.config.js 已固定生产默认值；此处打印用于日志审计。
post_json /api/sync
...
if (d.version !== '1.3.0') throw new Error(`dashboard version != 1.3.0: ${d.version}`);
