...
test('COUNT_KEYS：基础字段 + 周会漏斗字段', () => {
  assert.equal(COUNT_KEYS.length, 15);
  assert.ok(COUNT_KEYS.includes('jkuv'));
  assert.ok(COUNT_KEYS.includes('aggregationUv'));
  assert.ok(COUNT_KEYS.includes('brandPageUv'));
  assert.ok(COUNT_KEYS.includes('modelPageUv'));
  assert.ok(COUNT_KEYS.includes('conditionUv'));
...
test('calcCountDelta：不传 keys 默认使用 COUNT_KEYS', () => {
  ...
  assert.equal(Object.keys(delta).length, COUNT_KEYS.length);
  assert.equal(delta.jkuv.abs, 20);
  assert.equal(delta.gmv.abs, 20000);
  assert.deepEqual(delta.conditionUv, { abs: null, pct: null });
});
