...
test('normalizeBoardMetricRecord: 飞书备份字段映射 + 百分比归一化 + week_start_date 转 ISO 周', () => {
  ...
  assert.equal(row.recycleDau, 810000);
...
test('board-sync: 只从本地 board_metrics CSV 生成 board-metrics.json', () => {
  ...
  try {
    fs.writeFileSync(
      ...
      [
        'week_start_date,APP日均 DAU,回收日均 DAU,回收入口 UV,聚合回收渗透率,聚合回收真实渗透率',
        '2026-06-29,5200000,910000,162000,3.12%,2.10%',
        '2026-07-06,3936778,810000,120000,3.05%,1.24%',
      ].join('\n'),
      'utf8'
    );
    ...
    assert.equal(cache.rows[1].recycleDau, 810000);
